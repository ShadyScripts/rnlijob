...
function TakeOutVehicle(vehicleInfo)
    ...
    QBCore.Functions.SpawnVehicle(vehicleInfo, function(veh)
        SetVehicleNumberPlateText(veh, "rnlijob"..tostring(math.random(1000, 9999)))
...
function MenuGarage()
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "rnlijob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('rnlijob:client:TakeOutVehicle', function(data)
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    PlayerJob = JobInfo
    TriggerServerEvent("ambulance:server:Setrnlijob")
end)
...
CreateThread(function()
    ...
    while true do
        local sleep = 2000
        if LocalPlayer.state.isLoggedIn and PlayerJob.name == "rnlijob" then
                    ...
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "rnlijobstash_"..QBCore.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "rnlijobstash_"..QBCore.Functions.GetPlayerData().citizenid)
                            end
...
CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if PlayerJob.name =="rnlijob" then
